...
CREATE TABLE LOS_RELACIONALES.cliente(
	...
	COD_AGENCIA bigint REFERENCES LOS_RELACIONALES.agencia(NRO_AGENCIA), -- AGREGADO
	NOMBRE nvarchar(255),
...
CREATE TABLE LOS_RELACIONALES.encuesta(
	...
	COD_AGENTE bigint REFERENCES LOS_RELACIONALES.agente(LEGAJO), -- AGREGADO
	COD_CLIENTE bigint REFERENCES LOS_RELACIONALES.cliente(CLIENTE_ID), --AGREGAGO
	FECHA_ENCUESTA date,
...
CREATE TABLE LOS_RELACIONALES.aeropuerto_salida(
	...
	AER_SAL_CIUDAD nvarchar(255),
	AER_SAL_PAIS nvarchar(255)
);
...
CREATE TABLE LOS_RELACIONALES.aeropuerto_llegada(
	...
	AER_LLEG_CIUDAD nvarchar(255),
	AER_LLEG_PAIS nvarchar(255)
);

CREATE TABLE LOS_RELACIONALES.vuelo(
	NRO_VUELO bigint IDENTITY PRIMARY KEY,
	COD_AEROLINEA nvarchar(255) REFERENCES LOS_RELACIONALES.aerolinea(CODIGO_AEROLINEA),  -- MODIFICADO
...
CREATE TABLE LOS_RELACIONALES.detalle_propuesta_hospedaje(
...
CREATE TABLE LOS_RELACIONALES.detalle_venta_hospedaje(
...
-- REVISAR NORMALIZACION DE NOMBRE Y POSIBLE JOIN CON PAIS
...
BEGIN
	...
	FROM (
		...
		JOIN LOS_RELACIONALES.pais p ON p.Nombre = c.pais
...
-- REVISAR SI TODOS LOS CLIENTES TIENEN DNI (NOT NULL)
...
BEGIN
	...
	FROM gd_esquema.Maestra m
		JOIN LOS_RELACIONALES.alianza a ON a.ALIANZA = m.Aerolinea_Alianza
	/*
	FROM gd_esquema.Maestra m
		LEFT JOIN LOS_RELACIONALES.alianza a ON a.ALIANZA = m.Aerolinea_Alianza
	*/
	WHERE Aerolinea_Codigo IS NOT NULL
		AND m.Aerolinea_Alianza IS NOT NULL; -- SE SACA SI PERMITE NULL
END;
GO
-- REVISAR SI HAY ALIANZAS CON NULL

-- REVISAR JOINS Y SI SE DEBE RELACIONAR CON PAIS
...
BEGIN
	...
	FROM gd_esquema.Maestra	m
		...
		JOIN LOS_RELACIONALES.ciudad c ON c.COD_PAIS = p.PAIS_ID
			AND c.NOMBRE = m.Aeropuerto_Salida_Ciudad
...
BEGIN
	...
	FROM gd_esquema.Maestra	m
		...
		JOIN LOS_RELACIONALES.ciudad c ON c.COD_PAIS = p.PAIS_ID
			AND c.NOMBRE = m.Aeropuerto_Llegada_Ciudad
...
BEGIN
	...
	WHERE Proveedor_Nombre IS NOT NULL
		AND Proveedor_Mail IS NOT NULL;		-- REVISAR QUE PUEDE TENER NULL
END;
GO
...
BEGIN
	...
	FROM gd_esquema.Maestra	m
		...
		JOIN LOS_RELACIONALES.ciudad c ON c.COD_PAIS = p.PAIS_ID
			AND c.NOMBRE = m.Hospedaje_Ciudad
	WHERE m.Hospedaje_Nombre IS NOT NULL;	-- REVISAR QUE DEBE TENER NULL
END;
GO
...
BEGIN
	...
	FROM gd_esquema.Maestra	m
		JOIN LOS_RELACIONALES.hospedaje h ON h.NOMBRE = m.Hospedaje_Nombre
			...
			AND h.PAIS = m.Hospedaje_Pais  -- REVISAR SI ES CORRECTO
	WHERE m.Habitacion_Nombre IS NOT NULL;	-- REVISAR QUE DEBE TENER NULL
END;
GO

-- REVISAR JOINS NO VALIDOS
...
BEGIN
	...
	SELECT
		...
		m.Excursion_Precio
	FROM gd_esquema.Maestra	m
		-- ASUMIMOS QUE NO HAY CIUDADES REPETIDAS
...
BEGIN
	...
	SELECT DISTINCT
		Aspecto_Aspecto -- POSIBLE MEJORA LTRIM(RTRIM(Aspecto_Aspecto))
	FROM gd_esquema.Maestra
...
BEGIN
	...
	SELECT
        Encuesta_Codigo_Encuesta,
        MAX(Encuesta_Fecha_Encuesta),
        MAX(Encuesta_Comentarios)
    FROM gd_esquema.Maestra
    WHERE Encuesta_Codigo_Encuesta IS NOT NULL
    GROUP BY Encuesta_Codigo_Encuesta;
	*/
END;
GO

-- REVISAR SI FUNCIONA REVISANDO SI DNI SON UNICOS
...
-- REVISAR SI HAY CIUDADES REPETIDAS EN DIFERENTES PAISES
...
BEGIN
	...
	FROM gd_esquema.Maestra	m
		...
		-- MAYOR SEGURIDAD:  ON LTRIM(RTRIM(c.NOMBRE)) = LTRIM(RTRIM(m.Detalle_Solicitud_Ciudad))
	WHERE m.Solicitud_Nro_Solicitud IS NOT NULL
		AND m.Detalle_Solicitud_Ciudad IS NOT NULL;
END;
GO

-- REVISAR SI LOS JOINS ESTAN BIEN (SI SE USAN LEFT JOIN)
...
BEGIN
	...
	FROM gd_esquema.Maestra m
		LEFT JOIN LOS_RELACIONALES.solicitud s ON s.NRO_SOLICITUD = m.Solicitud_Nro_Solicitud
		LEFT JOIN LOS_RELACIONALES.encuesta en ON en.CODIGO_ENCUESTA = m.Encuesta_Codigo_Encuesta
		LEFT JOIN LOS_RELACIONALES.estado_propuesta ep ON ep.ESTADO = m.Propuesta_Estado
...
BEGIN
	...
	FROM gd_esquema.Maestra	m
		JOIN LOS_RELACIONALES.aerolinea a ON a.COD_AEROLINEA = m.Aerolinea_Codigo
...
-- REVISAR SI HAY QUE EXPANDIR EL JOIN DE VUELO
...
BEGIN
	...
	FROM gd_esquema.Maestra	m
		...
		JOIN LOS_RELACIONALES.vuelo v ON v.COD_AEROLINEA = m.Aerolinea_Codigo
			AND v.COD_AEROPUERTO_SALIDA = m.Aeropuerto_Salida_Codigo
			AND v.COD_AEROPUERTO_LLEGADA = m.Aeropuerto_Llegada_Codigo
...
-- REVISAR JOINS
CREATE PROCEDURE migracion_detalle_propuesta_hospedaje
AS
BEGIN
	INSERT INTO LOS_RELACIONALES.detalle_propuesta_hospedaje( 
		COD_PROPUESTA, 
		COD_HABITACION, 
	...
	FROM gd_esquema.Maestra	m
		JOIN LOS_RELACIONALES.propuesta p ON p.NRO_PROPUESTA = m.Propuesta_Nro_Propuesta
		JOIN LOS_RELACIONALES.hospedaje hp ON hp.NOMBRE = m.Hospedaje_Nombre
			AND hp.DIRECCION = m.Hospedaje_Direccion 
			AND hp.PAIS = m.Hospedaje_Pais  -- REVISAR SI ES CORRECTO
		JOIN LOS_RELACIONALES.habitacion h ON h.COD_HOSPEDAJE = hp.CODIGO_HOSPEDAJE
			AND h.NOMBRE = m.Habitacion_Nombre
...
BEGIN
	...
	FROM gd_esquema.Maestra	m
		...
		JOIN LOS_RELACIONALES.canal_venta cv ON UPPER(LTRIM(RTRIM(cv.CANAL_VENTA))) = UPPER(LTRIM(RTRIM(m.Venta_Canal_Venta)))
		JOIN LOS_RELACIONALES.medio_pago mp ON UPPER(LTRIM(RTRIM(mp.MEDIO_PAGO))) = UPPER(LTRIM(RTRIM(m.Venta_Medio_Pago)))
...
AS
BEGIN
	INSERT INTO LOS_RELACIONALES.detalle_venta_hospedaje(
	...
	FROM gd_esquema.Maestra m
		JOIN LOS_RELACIONALES.venta v ON v.NRO_VENTA = m.Venta_Nro_Venta
		JOIN LOS_RELACIONALES.hospedaje hos ON hos.NOMBRE = m.Hospedaje_Nombre
			AND hos.DIRECCION = m.Hospedaje_Direccion
			AND hos.PAIS = m.Hospedaje_Pais  -- REVISAR SI ES CORRECTO
		JOIN LOS_RELACIONALES.habitacion h ON h.COD_HOSPEDAJE = hos.CODIGO_HOSPEDAJE
			AND h.NOMBRE = m.Habitacion_Nombre 
...
BEGIN TRY
	...
	EXECUTE migracion_detalle_propuesta_hospedaje
