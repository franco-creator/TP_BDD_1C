...
CREATE TABLE LOS_RELACIONALES.detalle_propuesta_habitacion(
	COD_RESERVA_HABITACION bigint IDENTITY PRIMARY KEY,
	COD_PROPUESTA bigint REFERENCES LOS_RELACIONALES.propuesta(NRO_PROPUESTA),
	COD_HABITACION bigint REFERENCES LOS_RELACIONALES.habitacion(HABITACION_ID),
	DT_PROP_HABITACION_FECHA_DESDE date,
	DT_PROP_HABITACION_FECHA_HASTA date,
	DT_PROP_HABITACION_CANT int,
	DT_PROP_HABITACION_PRECIO decimal(18,2),
	DT_PROP_HABITACION_SUBTOTAL decimal(18,2)
);
...
CREATE TABLE LOS_RELACIONALES.detalle_venta_habitacion(
	DT_VENTA_HAB_COD_RESERVA nvarchar(255) PRIMARY KEY,
	COD_VENTA bigint REFERENCES LOS_RELACIONALES.venta(NRO_VENTA),
	COD_HABITACION bigint REFERENCES LOS_RELACIONALES.habitacion(HABITACION_ID),
	DT_VENTA_HABITACION_FECHA_DESDE date,
	DT_VENTA_HABITACION_FECHA_HASTA date,
	DT_VENTA_HABITACION_CANTIDAD int,
	DT_VENTA_HABITACION_PRECIO_UNITARIO decimal(18,2),
	DT_VENTA_HABITACION_SUBTOTAL decimal(18,2)
);
...
----- REVISAR NORMALIZACIONES ( UPPER(LTRIM(RTRIM(nvarchar))) )
...
-- SI PERMITE NULL EN ALIANZA, HACER LEFT JOIN
...
-- REVISAR SI HAY QUE EXPANDIR JOINS
...
BEGIN
	...
	FROM gd_esquema.Maestra	m
		JOIN LOS_RELACIONALES.hospedaje h ON h.NOMBRE = m.Hospedaje_Nombre
			...
			AND h.PAIS = m.Hospedaje_Pais
		-- -- AGREGAR JOINS SI SON NECESARIOS
	WHERE m.Habitacion_Nombre IS NOT NULL;
END;
GO

-- POTENCIAL PROBLEMA DE RELACION CON CIUDAD DE EXCURSION (CORREGIR)
...
BEGIN
	...
	SELECT DISTINCT
		...
		m.Excursion_Precio
	FROM gd_esquema.Maestra	m
		JOIN LOS_RELACIONALES.ciudad c ON c.NOMBRE = m.Hospedaje_Ciudad   -- REVISAR SI ES CORRECTO ASUMIR ESTO
...
-- CORREGIR Y REVISAR DER (NO TIENE PAIS EN MAESTRA)
...
BEGIN
	...
	FROM gd_esquema.Maestra	m
		...
		JOIN LOS_RELACIONALES.habitacion h ON h.COD_HOSPEDAJE = hp.CODIGO_HOSPEDAJE
			...
			-- REVISAR JOINS SI SON CORRECTOS
	WHERE m.Propuesta_Nro_Propuesta IS NOT NULL
...
BEGIN
	...
	FROM gd_esquema.Maestra m
		...
		JOIN LOS_RELACIONALES.propuesta p ON p.NRO_PROPUESTA = m.Propuesta_Nro_Propuesta
	WHERE m.Venta_Nro_Venta IS NOT NULL
		AND m.Propuesta_Nro_Propuesta IS NOT NULL; -- REVISAR SI HAY VENTAS SIN PROPUESTAS
END;
GO
